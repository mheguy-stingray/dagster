...
## Deciding on persistant vs ephemeral airflow database

The dagster airflow migration tooling supports two methods for persisting airflow metadatabase state. By default it will us an ephemeral database that is scoped to every job run and thrown away as soon a job run terminates. You can also configure a persistant database that will be shared by all job runs. This tutorial uses the ephemeral database but you will want to use the persistant database option if you need the following features:
...
If you have complex airflow DAGs you will likely want to have a persistant database when making your migration.

You can configure your persistant airflow database by providing an `airflow_db` to the resource_defs param of the dagster-airflow api's.
